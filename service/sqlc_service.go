...
func (s *SQLcService) SelectAll() ([]repository.ListCustomersRow, error) {
	...
	//logger := rkentry.GlobalAppCtx.GetLoggerEntry("my-logger")
	s.logger.Info("Call SQLcService.SelectAll")

	queries := repository.New(s.conn())
	cusList, err := queries.ListCustomers(s.ctx)
	if err != nil {
		s.logger.Info(fmt.Sprintf("SelectAll : ", err.Error()))
	}
	s.logger.Info(fmt.Sprintf("Call SQLcService.SelectAll length: %v", len(cusList)))
	return cusList, err
}
...
func (s *SQLcService) InsertNewCustomer(cus *CustomerRecord) (int32, error) {
	...
	if err != nil {
		s.logger.Info(fmt.Sprintf("InsertNewCustomer: %v", err.Error()))
		return -1, err
	}
...
func (s *SQLcService) UpdateCustomer(cus *CustomerRecord) (int32, error) {
	...
	if err != nil {
		s.logger.Info(fmt.Sprintf("UpdateCustomer : %v", err.Error()))
		return -1, err
	}
...
func (s *SQLcService) DeleteCustomer(cusId int32) (int32, error) {
	...
	if err != nil {
		s.logger.Info(fmt.Sprintf("DeleteCustomer : %v", err.Error()))
		return 0, err
	}
	return 1, nil
}
